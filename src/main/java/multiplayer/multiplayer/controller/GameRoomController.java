    ...
    public ResponseEntity<GameRoomJoinDTO> joinGameRoom(@PathVariable String gameRoomId, @RequestBody String clientId) {
        if (gameRoomService.getGameRoomById(gameRoomId).getGameRoomStatus().equals(GameState.NOT_STARTED)) {
            ...
            if (player == null) {
                ...
                // Let subscribers of /topic/game/{gameRoomId}/playerjoin that a new player has
                // joined, and what color, (and the previous players' colors)
                List<String> playerColors = gameroom.getPlayers().values().stream()
                        .sorted((p1, p2) -> p1.getCreatedAt().compareTo(p2.getCreatedAt())) // Make it easier to see
                                                                                            // what color joined, as
                                                                                            // they will appear last in
                                                                                            // the list
                        .map(p -> p.getColor())
                        .toList();
